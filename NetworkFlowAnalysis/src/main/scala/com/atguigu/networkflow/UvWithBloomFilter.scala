...
import com.atguigu.networkflow.UniqueVisitor.getClass
...
object UvWithBloomFilter {
  def main(args: Array[String]): Unit = {
    ...
    uvStream.print("result")


    env.execute("uv job with bloom filter")
  }
}
...
class MyBloomFilter(size: Long) extends Serializable {
  //指定布隆过滤器的位图大小 由外部参数指定 位图存在redis中
...
class UvCountResultWithBloom extends ProcessWindowFunction[(String, Long), UvCount, String, TimeWindow] {
  //创建redis连接和布隆过滤器
  lazy val jedis = new Jedis("hadoop202", 6379)
  lazy val bloom = new MyBloomFilter(1 << 29) //1亿id，1亿个位大约需要2^27, 设置大小2^29
